...
const TradeHistory = () => {
  ...
  const handleCloseTrade = async (tradeId) => {
    try {
      ...
      else toast.error(res?.data?.message || "Failed to close trade");
    } catch (err) { toast.error(err?.response?.data?.message || `Server error`); }
  ...
  return (
    <div className="w-full h-full min-h-screen flex flex-col font-poppins relative overflow-hidden">
      <ToastContainer />
      <div className="absolute top-1/3 left-1/4 w-72 h-72 bg-blue-900/10 blur-[100px] pointer-events-none rounded-full"></div>
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 relative z-10 glass-panel p-5 rounded-2xl">
        <div className="flex items-center gap-4">
          <div className="p-2 border border-brand-gold/30 rounded-xl shadow-glow-gold/20 bg-brand-gold/10">
            <FaExchangeAlt className="text-brand-gold text-xl" />
          </div>
          <div>
                  ...
                  return (
                    <tr key={t._id || i}>
                      ...
                      <td className="text-gray-300">
                        {t.status === "CLOSED" ? t.exitPrice : current ? current : <span className="text-gray-600 text-xs animate-pulse">Loading...</span>}
                      </td>
                      ...
                      <td className="text-center">
                        <span className="px-2 py-1 text-xs rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">{t.status || "Active"}</span>
                      </td>
                      <td className="text-center">
                        {t.status !== "CLOSED" && (
